            ...
            try:
                with open(cache_file, 'r') as f:
                    ...
                    if isinstance(cached_data, list):
                        ...
                        return cached_data.get('data', cached_data.get('results', []))
                    return []
            except Exception:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                if 'data' in data:
                    ...
                    # If no known key, log what we got
                    st.warning(f"Unexpected response format for {date}. Keys: {list(data.keys())}")
                    return []
            else:
            ...
            if e.response.status_code == 401:
                st.error("❌ API Authentication Error: Invalid API key")
            elif e.response.status_code == 429:
                st.error("❌ API Rate Limit: You've exceeded your quota")
            elif e.response.status_code == 422:
                st.warning(f"⚠️ Invalid request for {date}: {e.response.text}")
            else:
                st.warning(f"HTTP Error {e.response.status_code} for {sport} on {date}")
            return []
    ...
    def fetch_historical_scores(self, sport: str, date: str) -> List[Dict]:
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                st.error("❌ API Rate Limit: Quota exceeded")
            else:
                st.warning(f"HTTP Error {e.response.status_code} fetching scores for {date}")
            return []
        except Exception as e:
    ...
    def build_historical_dataset(self, sport: str, days_back: int = 90) -> pd.DataFrame:
        ...
        for i in range(days_back):
            ...
            try:
                ...
                # Log what we got
                if i == 0:  # First day - show detailed info
                    st.info(f"📊 Sample data from {date}:")
                    st.info(f"  Odds data: {len(odds_data) if odds_data else 0} games")
                    st.info(f"  Scores data: {len(scores_data) if scores_data else 0} games")
                
                total_games_found += len(odds_data) if odds_data else 0
                ...
                for game in scores_data:
                    if isinstance(game, dict) and 'id' in game:
                        scores_map[game['id']] = game
                
                total_games_with_scores += len(scores_map)
                ...
                for game in odds_data:
                    ...
                    game_id = game.get('id')
                    if not game_id or game_id not in scores_map:
                        continue
                ...
                try:
                    ...
                    if not scores or len(scores) < 2:
                        continue
                    home_score = scores[0].get('score') if isinstance(scores[0], dict) else None
                    away_score = scores[1].get('score') if isinstance(scores[1], dict) else None
                ...
                # Extract odds with safety checks
                home_odds = None
                away_odds = None
                home_spread = None
                away_spread = None
                ...
                if home_odds and away_odds:
                    try:
                        ...
                        home_won = 1 if home_score_val > away_score_val else 0
                        away_won = 1 if away_score_val > home_score_val else 0
                        ...
                        if home_decimal and away_decimal:
                            ...
                            records.append({
                                ...
                                'total_score': home_score_val + away_score_val,
                                'home_won': home_won,
                                'away_won': away_won,
                        ...
                        # Skip this game if we can't process the data
                        continue
        ...
        - Days requested: {days_back}
        - Total games found: {total_games_found}
        - Games with scores: {total_games_with_scores}
        - Games successfully processed: {total_games_processed}
        ...
        if len(records) == 0:
            ...
            Possible causes:
            1. **No games during this period** - Try a different date range
            2. **API subscription doesn't include historical data** - Check your plan
            3. **Sport not in season** - Try a different sport or date range
            4. **API format changed** - Check the diagnostic output above
            
            Try:
            - Different sport (NFL, NBA, MLB)
            - Recent dates (last 7-30 days)
            - Fewer days (7-14 instead of 30+)
            """)
